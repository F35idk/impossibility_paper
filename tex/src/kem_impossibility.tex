...
\begin{theorem}[Impossibility of tight \OWECPA reductions]\label{thm:main}
  ...
  with \((1-\delta)\)-correctness, encapsulation key space \keyspace
  and secret keys of bit length at most \(q\) for some polynomial \(q : \N \to \N\),
  ...
  \end{equation}
  for all sufficiently large \(\mu \ge 0\).
\end{theorem}

\TODO{try to optimize the constant \(5\), i.e by improving Corollary \ref{corollary:equiv}}.
...
If, say, \(\rho \ge 10 \mu q\), then we obtain \(\ell \ge (1 - 1/\abs{\keyspace}) \mu / 2 - \negl\),
which is linear in the number of users \(\mu\) in the \OWECPA game.
% In particular, we must have \(\rho \ge \mu\) for the result to imply a linear loss in the number of users \(\mu\).
...
% Indeed, reductions with a loss factor \(\ell = \Theta(\rho)\) exist (\TODO{either refer to such a scheme, or show one in the appendix}).
...
\begin{namedproof}{Proof of Theorem \ref{thm:main}}
  ...
  We may assume that \(\rho \ge 5 \mu q\), or else \eqref{eq:main} is trivial.
  Setting \(t = \rho / 5\), \(\alpha = q/t = 5 q / \rho \le 1/\mu\) gives
  \begin{align}
    ...
    & \le 4^{q} \cdot \left( \frac{1}{5 - 1/\mu} \right)^{q} \notag\\
    & = \negl, \notag
  \end{align}
  ...
  \begin{align}
    \frac{1/\mu + \negl}{1/\mu + \alpha + \negl} \le \frac{1/\mu + \negl}{1/\mu + \negl} = 1 + \negl, \notag
  \end{align}
  and Taylor's theorem along with \(\alpha \le 5q / \rho\) gives
  \begin{align}
    \frac{1}{1/\mu + \alpha + \negl} \ge \mu - \mu^{2}(\alpha + \negl) & = \mu - \mu^{2} \alpha - \negl \notag\\
    & \ge \mu - \mu^{2} 5 q / \rho - \negl, \notag
  \end{align}
  which inserted into \eqref{ineq:insert} yields
  \begin{align}
    \frac{\epsilon_{\advA}}{\epsilon_{\redR^{\advA}}} \ge (1 - 1/\abs{\keyspace})(\mu - 5 \mu^{2} q / \rho) - 1 - \negl. \notag
  \end{align}
  Noting that \(T_{\redR} \ge T_{\advA}\),
  which follows since \redR is a simple reduction and
  hence runs the adversary \advA to completion,
  we get
  \[
    \ell \ge \frac{\epsilon_{\advA}}{\epsilon_{\redR^{\advA}}} \ge (1 - 1/\abs{\keyspace})(\mu - 5 \mu^{2} q / \rho) - 1 - \negl, \notag
  \]
  as desired.
\end{namedproof}
